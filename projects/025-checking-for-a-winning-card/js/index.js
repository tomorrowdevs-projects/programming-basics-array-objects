...
// The function accepts the Bingo card and the number called as arguments.
function getCalledNumber(card, number) {
    for (let key in card) {
        // Search the called numbers in each column of the bingo card ane replace with 0 if present
        if (card[key].includes(number)) {
            card[key][card[key].indexOf(number)] = 0;
        }
    }
}

let result = {};
result = randomBingoCard();

const calledNumbers = new Set();
for (let j = 1; j <= 75; j++) {
    let randomNumber = Math.floor(Math.random() * 75 + 1);
    calledNumbers.add(randomNumber);
    getCalledNumber(result, randomNumber);
}
