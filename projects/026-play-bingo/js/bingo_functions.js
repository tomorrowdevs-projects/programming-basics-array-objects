...
function markBingoCard(bingoCard, extractNumber) {
    ...
    for (const key in bingoCard) {
        ...
        const index = bingoCard[key].indexOf(extractNumber);
        if (index !== -1) {
...
// // This part of code let me to create different players with one Bingo Card for once.
...
// for (const key in ingameBingoCard) {
...
//             for (const player in ingameBingoCard) {
